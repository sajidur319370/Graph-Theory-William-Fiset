...
public class TspDynamicProgrammingIterative {
		...
		for (int r = 3; r <= N; r++) {
			for (int subset : combinations(r, N)) {
				...
				if (notIn(start, subset)) {
					...
					System.out.println("Continue111!!!!!!!!!!!!!");
					System.out.println("start111111: " + start);
					continue;

				}

				System.out.println("Continue2222!!!!!!!!!!!!!");

				for (int next = 0; next < N; next++) {
					if (next == start || notIn(next, subset)) {
						System.out.println("start: " + start);
						System.out.println("next: " + next);
						System.out.println("Continue3333!!!!!!!!!!!!!");
						continue;
					}

					int subsetWithoutNext = subset ^ (1 << next);
					System.out.println("SubNext :" + subsetWithoutNext);
					double minDist = Double.POSITIVE_INFINITY;
					for (int end = 0; end < N; end++) {
						if (end == start || end == next || notIn(end, subset)) {
							System.out.println("end next: " + next);
							continue;
						}
	...
	private static void combinations(int set, int at, int r, int n, List<Integer> subsets) {
		...
		System.out.println("Element to pick: " + elementsLeftToPick);
		...
		if (r == 0) {
			...
			for (int i = at; i < n; i++) {
				...
				set ^= (1 << i);
				System.out.println("Before Recursion Set: " + set);

				combinations(set, i + 1, r - 1, n, subsets);

				// Backtrack and try the instance where we did not include this element
				set ^= (1 << i);
				System.out.println("After Recursion Set: " + set);
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		/*
		 * distanceMatrix[5][0] = 10; distanceMatrix[1][5] = 12; distanceMatrix[4][1] =
		 * 2; distanceMatrix[2][4] = 4; distanceMatrix[3][2] = 6; distanceMatrix[0][3] =
		 * 8;
		 */

		/*
		 * distanceMatrix[3][0] = 13; distanceMatrix[2][3] = 12; distanceMatrix[1][2] =
		 * 7; distanceMatrix[0][1] = 8;
		 */

		distanceMatrix[5][0] = 16;
		distanceMatrix[5][2] = 7;
		distanceMatrix[4][5] = 21;
		distanceMatrix[5][4] = 21;
		distanceMatrix[3][1] = 9;
		distanceMatrix[3][4] = 12;
		distanceMatrix[4][3] = 12;
		distanceMatrix[2][3] = 15;
		distanceMatrix[1][2] = 16;
		distanceMatrix[0][1] = 34;
