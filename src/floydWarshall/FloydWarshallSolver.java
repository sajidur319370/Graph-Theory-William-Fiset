...
public class FloydWarshallSolver {
	...
	/**
	 * As input, this class takes an adjacency matrix with edge weights between
	 * nodes, where POSITIVE_INFINITY is used to indicate that two nodes are not
	 * connected.
	 *
	 * <p>
	 * NOTE: Usually the diagonal of the adjacency matrix is all zeros (i.e.
	 * matrix[i][i] = 0 for all i) since there is typically no cost to go from a
	 * node to itself, but this may depend on your graph and the problem you are
	 * trying to solve.
	 */
	...
	/**
	 * Runs Floyd-Warshall to compute the shortest distance between every pair of
	 * nodes.
	 *
	 ...
	 *         If 'start' and 'end' are not connected return an empty array. If the
	 *         shortest path from 'start' to 'end' are reachable by a negative cycle
	 *         return -1.
	 */
	...
	public static void main(String[] args) {
		...
		// The shortest path from node 0 to node 3 DOES NOT EXIST (node 0 doesn't reach
		// node 3)
		// The shortest path from node 0 to node 4 HAS AN ∞ NUMBER OF SOLUTIONS!
		// (negative cycle case)
		// The shortest path from node 0 to node 5 HAS AN ∞ NUMBER OF SOLUTIONS!
		// (negative cycle case)
		// The shortest path from node 0 to node 6 is: [0 -> 1 -> 2 -> 6]
		// The shortest path from node 1 to node 0 DOES NOT EXIST (node 1 doesn't reach
		// node 0)
		// The shortest path from node 1 to node 1 is: [1]
		// The shortest path from node 1 to node 2 is: [1 -> 2]
		// The shortest path from node 1 to node 3 DOES NOT EXIST (node 1 doesn't reach
		// node 3)
		// The shortest path from node 1 to node 4 HAS AN ∞ NUMBER OF SOLUTIONS!
		// (negative cycle case)
		// The shortest path from node 1 to node 5 HAS AN ∞ NUMBER OF SOLUTIONS!
		// (negative cycle case)
		// The shortest path from node 1 to node 6 is: [1 -> 2 -> 6]
		// The shortest path from node 2 to node 0 DOES NOT EXIST (node 2 doesn't reach
		// node 0)
		// ...
	}
}